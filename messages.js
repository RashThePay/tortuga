...
const msg = {
  ...
  teamDM: (team) =>
    `🏴‍☠️ *جزیره‌ی گنج*\n\nتیم شما: *${TEAM_NAMES[team]}*\n\nاین اطلاعات محرمانه است. به کسی نگویید!`,
  ...
  dutchDM:
    ...
    '*شرایط برد شما:*\n' +
    '• اگر ناخدای یک کشتی باشید، برنده‌اید.\n' +
    '• اگر در کشتی‌ای باشید که گنج تیم برنده بیشتر است، برنده‌اید.\n' +
  ...
  dayStart: (round) =>
    ...
    `/attack - دستور حمله (ناخدا یا معاون)\n` +
    `/maroon - فرستادن به خشکی (ناخدا)\n` +
    `/mutiny - شورش (معاون)\n` +
    `/move - جابه‌جایی گنج (پادو)\n` +
  ...
  notOnShip: '⚠️ شما در هیچ کشتی‌ای نیستید.',
  notOnIsland: '⚠️ شما در جزیره نیستید.',
  notGovernor: '⚠️ فقط حاکم جزیره می‌تواند این کار را کند.',

  attackOrdered: (name, ship, role) =>
    `⚔️ ‏${name} (${role} ${shipLabel(ship)}) دستور حمله داد!`,
  ...
  attackFailedDeposed: (name) =>
    `⚔️ حمله شکست خورد! ‏${name} عزل شده و به جزیره فرستاده شد.`,
  mutinyFailedDeposed: (name, ship) =>
    `⚔️ شورش شکست خورد! ‏${name} (معاون ${shipLabel(ship)}) عزل شده و به جزیره فرستاده شد.`,
};
